...
| User asks for | Route | Tier | Build stages | Minimum references |
|---|---|---|---|---|
| 快速内部初稿、brainstorming、培训草稿、只要先出一版 | `express` | `express` / `internal-draft` | `S1-minimal → S2-simple → S4-basic → S6-draft` | `agent-behavioral-guardrail.md`, `task-tier-protocol.md`, `v2-capability-router.md`, `visual-qa-protocol.md` |
| 美化、润色、改标题、优化少量页面 | `targeted-edit` / `optimize` | `quick-polish` | `S1-minimal → S2-patch → S3-optional → S4-basic → S6-polish` | `agent-behavioral-guardrail.md`, `task-tier-protocol.md`, `agent-runtime.md`, `language-discipline.md`, `visual-qa-protocol.md` |
| 从材料生成结构化 deck 或给经理/合伙人初审 | `create` / `optimize` | `partner-ready` | `S0 → S1 → S1.6 → S2.5 → S2.6 recommended → S2 → S3 → S4 → S5 → S6` | `agent-behavioral-guardrail.md`, `confirmation-state-machine.md`, `confirmation-log-standard.md`, `strategy-brief.md`, `source-digest-standard.md`, `consulting-storyline-standard.md`, `storyline-page-planning.md`, `template-catalog.md`, `review-loop.md` |
| 客户正式交付、董事会、管理层、投委会、监管汇报 | `create` / `optimize` | `client-ready` | `S0 → S1 → S1.5 → S1.6 → S2.5 → S2.6 → S2 → S3 → S4 → S5 → S6` | `agent-behavioral-guardrail.md`, `confirmation-state-machine.md`, `confirmation-log-standard.md`, `client-delivery-standard.md`, `logic-gate-checklist.md`, `conclusion-evidence-matrix.md`, `data-lineage-protocol.md`, `professional-chart-rulebook.md`, `editability-check.md`, `visual-qa-protocol.md`, `deck-quality-scorecard.md` |
| 搭建/复刻/自动化 PPT 生产体系或模板库 | `pipeline` | `pipeline` | `S0 → S1 → S1.5 → S1.6 → S2.5 → S2.6 → S2 → S3 → S4 → S5 → S6 → regression` | `agent-behavioral-guardrail.md`, `progressive-loading-protocol.md`, `tier-stage-matrix.md`, `workflow-san-pipeline.md`, `template-manifest.md`, `artifact-schema-library.md`, `visual-profile-registry.md`, `sample-regression-test.md`, `profile-regression-matrix.md` |

默认不跳过访谈和框架确认。只有两类情况可以跳过：

1. `quick-polish` / `targeted-edit`：只改错字、字体、颜色、对齐、个别页面或明确字段。
2. 用户明确写出类似“跳过提问，直接生成最终 PPT / skip questions and build”的强指令。

“按现有材料处理”“帮我优化”“整体升级”“继续做一版”不视为跳过确认；仍必须先提问并等待用户回答。

## Core Workflow

1. **Route**
   - 每次任务先读 [agent-behavioral-guardrail](references/agent-behavioral-guardrail.md)，避免把“继续做一版/按材料处理”误判为确认信号。
   - 若任务需要多阶段执行，读 [progressive-loading-protocol](references/progressive-loading-protocol.md)，只加载当前 tier 和 stage 必需 reference。
   - 先判断任务是 `express`、`create`、`optimize`、`targeted-edit` 还是 `pipeline`。
   ...
   - PRD V2 能力按需读取 [v2-capability-router](references/v2-capability-router.md)：不要在小任务中默认加载完整 V2 能力。
...
2. **Ask Before Building**
   - 对 `create`、整体 `optimize`、`partner-ready`、`client-ready`、`pipeline` 项目，第一步必须读 [strategy-brief](references/strategy-brief.md)，先向用户提出 4-8 个中文确认问题并等待回答。即使材料看起来完整，也要把推断写成假设请用户确认。
   - 任何确认节点必须读 [confirmation-state-machine](references/confirmation-state-machine.md)：只有用户明确确认、确认带修改或明确跳过确认，才能进入下一阶段。
   - `partner-ready` 以上项目必须读 [confirmation-log-standard](references/confirmation-log-standard.md)，维护 `confirmation_log.json`，记录 CN1/CN2/CN3 的用户信号、状态和锁定产物。
   ...
   - 禁止因为“材料足够”“用户说按材料处理”“之前已经聊过类似项目”而直接生成。除非用户明确要求跳过提问并直接生成最终 PPT，否则必须停在问题或框架确认节点。
...
4. **Design The Story**
   ...
   - 进入逐页大纲前再次核对 [structure-first-confirmation-protocol](references/structure-first-confirmation-protocol.md)：如果整体框架尚未被用户确认，只能输出 `framework_confirmation`，不能继续生成细节。
...
5. **Choose Layout And Visual System**
   ...
   - 生成 `preset_map.json` 前必须读 [layout-lock-protocol](references/layout-lock-protocol.md)，按 visual profile 白名单锁定 page family；不得临时发明自由布局。
   ...
   - 用户提供或要求使用 `ppt版式/` 参考版式时，必须读 [reference-layout-library](references/reference-layout-library.md)、[reference-layout-analysis-framework](references/reference-layout-analysis-framework.md) 和 [scenario-layout-selector](references/scenario-layout-selector.md)，先分析版面布局、字体、字号、结构、图表、表格和图片规则，再按场景选择候选版式。
...
- 每次任务先遵守 `agent-behavioral-guardrail.md`：要求等待确认时，当前回复必须停在确认请求，不得继续后续 stage。
- 除 `quick-polish`、`targeted-edit` 或用户明确要求“跳过提问并直接生成最终 PPT”外，完整生成和整体重构必须先问 4-8 个确认问题，等用户回答后再确认整体结构和分析框架，最后才生成逐页内容。
...
- 可跳过完整访谈和逻辑 gate，但必须保留标题为结论、基本来源边界、字号/溢出/对齐 QA。
...
- 构建前必须确认整体框架；若用户要求跳过确认，必须在交付说明中标记 `assumed_user_requested_direct` 和未确认风险。
...
- 复杂项目必须先形成中间产物：`brief.json`、`source_digest.md`、`confirmation_log.json`、`conclusion_evidence_matrix.json`、`argument_map.json`、`data_pool.json`、`lineage_map.json`、`insights.json`、`insight_layout_map.json`、`content_density_report.json`、`reference_layout_profile.json`、`title_spine.md`、`storyline_map.json`、`outline.json`、`design_system.json`、`preset_map.json`、`layout_analysis_report.json`、`html_preview_report.json`、`chart_data/`、`image_assets.json`、`visual_intent/`、`review_report.json` 或等价记录。
...
- V2 capability routing: [v2-capability-router](references/v2-capability-router.md)
- Workflow SAN: [workflow-san-pipeline](references/workflow-san-pipeline.md)
- Strategy questions: [strategy-brief](references/strategy-brief.md), [structure-first-confirmation-protocol](references/structure-first-confirmation-protocol.md)
- Data and context: [source-digest-standard](references/source-digest-standard.md), [data-pipeline](references/data-pipeline.md), [data-lineage-protocol](references/data-lineage-protocol.md), [content-freshness-and-evidence](references/content-freshness-and-evidence.md), [context-enrichment](references/context-enrichment.md)
