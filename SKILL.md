...
- 每个任务必须标记执行档位：`quick-polish` 不得声称 client-ready；`partner-ready` 不得声称 client-ready；`client-ready` 必须通过逻辑 gate、语言 gate、可编辑 gate 和 contact sheet/profile 回归检查。
