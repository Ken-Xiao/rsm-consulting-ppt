...
## Public Citation Risk Check

`client-ready` 且客户为上市公司、金融机构、监管敏感主体时，增加公开引用安全检查。本检查不是法律审查，只识别“如果被直接引用到公告、监管报告或外部材料中可能产生误读”的表达。

重点扫描：

- 可能被理解为业绩预告、盈利承诺或投资建议的措辞。
- 可能被理解为监管承诺、法律结论或合规保证的措辞。
- 未加前提的估值、交易、风险和处罚判断。
- 将项目组假设、访谈反馈或第三方估算写成确定事实。
    ...
    "items": [
      {
        ...
        "field": "claim_title",
        "risk": "可能被误读为业绩承诺",
        "rewrite": "基于现有信息，利润改善可持续性仍需结合承保质量和投资收益稳定性验证"
      }
    ]
  }
}
